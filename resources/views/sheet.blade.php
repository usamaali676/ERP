...
$permcmnt = App\Models\perm::where('role_id', $user->role_id)->where('name', "Comments")->first();
?>
@endif
    <div class="content content--top-nav">
        <div class="intro-y flex items-center mt-8">
            ...
            @if (auth()->user()->is_closer == 1)
            <div class="export-btn px-2">
                <a class="btn btn-primary" href="{{ route('closed-sales') }}" role="button">
            ...
            <div class="import-btn">
                <a class="btn btn-outline-primary" href="{{ route('sale.create') }}" role="button">
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table id="example" class="table table-report">
                                            ...
                                            <tbody>
                                                ...
                                                @if(Auth::check())
                                                @if (auth()->user()->id == $item->agent_id  || auth()->user()->role_id == 1)
                                                <tr >
                                                    <td>{{$srno++}}</td>
                                                    ...
                                                    {{-- <td>{{$item->date}}</td> --}}
                                                     ...
                                                     <td>@if($permsheet->edit == 1)<a class="btn btn-warning mr-1 mb-2" href="{{ route('sale.edit',$item->id) }}" > <i data-lucide="edit" style="color: #fff" class="w-5 h-5"></i> </a>@endif @if($permsheet->view == 1)<a href="{{route('sale-datail', $item->id)}}" class="btn btn-success mr-1 mb-2"> <i data-lucide="eye" class="w-5 h-5" style="color: #fff"></i> </a>@endif @if($permsheet->delete == 1)<a href="{{route('sale.conf-delete', $item->id)}}" class="btn btn-danger mr-1 mb-2"> <i data-lucide="trash" class="w-5 h-5"></i> </a>@endif @if($permcmnt->create == 1)<a href="{{route('cmnt.create', $item->id)}}" class="btn btn-primary mr-1 mb-2"> <i data-lucide="message-circle" class="w-5 h-5"></i> </a>@endif </td>
                                                </tr>
                                                @endif
                                                @endif
                                                @endforeach
